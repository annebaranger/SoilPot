...
Refit model with accurate traits
on the way of computing quantile inside.outside euforgen distribution: when do i take only presence/absence?
...
df.traits.all=df.P50%>% 
  ...
  rename_with(.cols=everything(),
                tolower) |> 
  select(species.binomial,group,p50.mu,p50.sd,p88.mu,p88.sd,lt50.mean,lt50.sd,bdd) |> 
...
c<-df.mod.select %>%
  ...
  mutate(pred=case_when(mod=="2sm"&xsm_name=="Hydraulic safety margins (MPa)"~k_int/((1+exp(-r_fsm*(0-t_fsm)))*
                                                            (1+exp(-r_hsm*(xsm_val-t_hsm)))),
                        mod=="2sm"&xsm_name=="Frost safety margins (°C)"~k_int/((1+exp(-r_hsm*(0-t_hsm)))*
                                                            (1+exp(-r_fsm*(xsm_val-t_fsm)))),
                        mod=="hsm"~k_int/(1+exp(-r_hsm*(xsm_val-t_hsm))),
                        mod=="fsm"~k_int/(1+exp(-r_fsm*(xsm_val-t_fsm)))
                        )
         ) |>
  ggplot(aes(xsm_val,pred,color=group,linesize=species.binomial))+ #color=prevalence,
  geom_vline(xintercept = 0,linetype="dashed")+
...
a<-df.mod.select %>% 
  left_join(df.species) |>
  left_join(df.range) |> 
  select(species.binomial,px.mu,lt50.mean,t_hsm,t_fsm,mod,psi_range,t_range,group) %>% 
  mutate(`HSM`=t_hsm/psi_range,
         FSM=t_fsm/t_range) |> 
  pivot_longer(cols=c("px.mu","lt50.mean"),names_to = "traits",values_to = "traits_val") %>% 
