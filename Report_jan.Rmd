...
```{r par_t_traits}
df.mod.select %>% 
  select(species.binomial,PX.mu,LT50.mean,t_hsm,t_fsm,mod,prevalence) %>% 
...
df.species |> 
  ...
  ggplot(aes(prevalence,shade_tolerance.mean,label=species.binomial))+
  geom_point()+
  geom_text()+
  geom_smooth(method="gam")
...
db.clim |> 
  ...
  summarise(min.hsm=quantile(hsm,probs=0.01),
           max.hsm=quantile(hsm,probs=0.99),
           med.hsm=median(hsm)) |> 
...
db.clim |> 
  ...
  filter(psi>(-10000)) |> 
  group_by(species.binomial,px.mu,prevalence) |> 
  summarise(q05.psi=quantile(hsm,probs=0.05)) |> 
...
db.clim |> 
  ...
  filter(psi>(-15000)) |>
  mutate(hsm_sc=hsm/px.mu) |> 
  ggplot(aes(psi,species.binomial))+
  geom_boxplot(outlier.shape = NA)
...
db.clim |> 
  ...
  summarise(min.hsm=quantile(psi,probs=0.01),
           max.hsm=quantile(psi,probs=0.99)) |> 
  mutate(hsm.width=max.hsm-min.hsm) |> 
  ggplot(aes(prevalence,hsm.width/1000,color=species.binomial))+
  geom_point()+
  geom_smooth(method='gam')+
  theme(legend.position = "none")
...
# Compute overlap with shade tolerance - test

```{r}
df <- data.frame(p = rep(1:4, each = 4), 
                 sp = c("toto", "tutu", "titi","tata"), 
                 pres = c(T, T, T, T, F, T, T, F,T, T, F, F,F,T,T,T),
                 shade = c(1, 2, 3,4))
df


#' Compute shade overlap
#' 
#' Shade overlap is the number of species with greater shade tolerance than 
#' the species shade tolerance present in a plot.
#' 
#' @param shade vector of shade tolerance (dbl) for all species
#' @param pres presence vector (lgl) for all species.
#' 
#' @examples 
#' shade_overlap(shade = 1:3, pres = c(TRUE, TRUE, TRUE))
#' shade_overlap(shade = 1:3, pres = c(TRUE, TRUE, FALSE))
#' shade_overlap(shade = 1:3, pres = c(TRUE, FALSE, TRUE))
#' 
#' @importFrom  checkmate assertNumeric assertLogical
#' 
shade_overlap <- function(shade, pres){
    
    # assertNumeric(shade, any.missing = FALSE)
    # assertLogical(pres, any.missing = FALSE, len = length(shade))
    
    # low <- lower.tri(outer(shade, shade))
    prem <- shade * pres
    
    overlap <- outer(prem, prem, ">") #* low
    res <- colSums(overlap) * pres
    return(res)
}

df %>% group_by(p) %>%
    mutate(overlap = shade_overlap(shade, pres) )


df.overlapshade=db.clim |>
  filter(presence==1) |> 
  select(node,species.binomial,presence) |> 
  left_join(df.species[,c("species.binomial","shade_tolerance.mean")]) |> 
  filter(!is.na(shade_tolerance.mean)) |> 
  group_by(node) |> 
