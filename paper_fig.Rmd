...
psi_eraday_real <- tar_read(psi_eraday_real,store="target_psi2")
psi_eraday_100 <- tar_read(psi_eraday_100,store="target_psi2")
...
``` {r }
predictor=c("tmin_era","psi_eraday_real","sgdd","wai","pet","map","prpet","mat")
db.clim.lm<- 
  db.clim |>  
  filter(presence==1) |> 
  ...
  summarise(quant05=quantile(value,probs=0.05,na.rm=TRUE)[[1]],
            quant95=quantile(value,probs=0.95,na.rm=TRUE)[[1]]) |> 
  ungroup() |> 
  mutate(lt50=scale(lt50),
         px=scale(px)) |> 
  pivot_longer(cols=c("quant05","quant95"),
               names_to="quant_name",
               values_to = "quant_val") |> 
  left_join(df.species[,c("species","taxa")]) 
...
``` {r fig2.withoutoutrangedspecies.2}
df.n <- df.mod.select %>% 
  filter(mod!="none") |> 
  left_join(df.quant.final) |> 
  filter(hsm.valid.3==TRUE&fsm.valid.2==TRUE) |> 
...
``` {r fig2.withoutoutrangedspecies3}
for(pred in predictor){
  print(pred)
  for(quant in c("quant05","quant95")){
    print(quant)
...
df.lm.overlap |> 
  # mutate(quant=recode(quant, quant05 = "5%", quant95 = "95%")) |> 
  mutate(pred=case_when(pred=="overlap_drought"~"Competitive dominance index \nfor drought tolerance",
                        pred=="overlap_shade"~"Competitive dominance index \nfor shade tolerance")
         ) |> 
  separate(col = par,
...
rm(df.traits,df.traits.all)
tar_load(df.mod.select.varb)
df.mod.select=df.mod.select.varb
rm(df.mod.select.varb)
```
...
df.n<- df.mod.select %>% 
  left_join(df.species) |>
    ...
    mutate(t_range=case_when(is.na(t_range)~mean(df.range$t_range,na.rm=TRUE),
         ...
         HSM=t_hsm/psi_range,
         FSM=t_fsm/t_range) |> 
  dplyr::select(group,species.binomial,mod,HSM,FSM) %>% 
  pivot_longer(cols=c("HSM","FSM"),names_to = "threshold",values_to = "traits_val") %>% 
...
df.mod.select %>% 
  ...
  mutate(n=n()) |> 
  ungroup() |> 
  ggplot(aes(threshold,threshold_val,fill=group))+
  geom_boxplot()+
  geom_text(data=df.n,
            mapping=aes(x=threshold,y=value,label=n),
            position = position_dodge(width=0.75),
...
#        width=10,
#        height = 6)
ggsave("figures/fig2_beta_gg.pdf",
       plot=plot,
       scale=2,
       dpi=600,
       width = 13,
...
df.range<-db.clim |> 
  ...
  group_by(species.binomial) |> 
  summarise(psi_range=max(psi)/1000-min(psi)/1000,
            t_range=max(frost.winter,na.rm=TRUE)-min(frost.winter,na.rm=TRUE))
...
threshold_prior=df.output.select |> select(species,mod) |> #extract model selection
  left_join(df.mod.output,by=c("species","mod"))  |> 
  select(species,mod,parameter) |> 
  filter(grepl("t_",parameter)) |> unique() |> 
  mutate(file=case_when(mod=="hsm"~paste0(species,"_fitIII_hsm.RData"),
                        mod=="fsm"~paste0(species,"_fitIII_fsm.RData"),
                        mod=="2sm"~paste0(species,"_fitIII.RData"),
                        mod=="none"~paste0(species,"_fitIII_none.RData")))

threshold_post=threshold_prior |> 
  left_join(df.range,by=c("species"="species.binomial")) |> 
...
# Loop over the files
  for (i in seq_along(threshold_prior$species)) {
    ...
    tryCatch({
      new_env <- new.env()
      load(file=paste0("fit_mod6/", threshold_prior$file[i]),envir=new_env)
...
threshold_post |> 
  left_join(df.species[,c("group","species.binomial")],by=c("species"="species.binomial")) |> 
  mutate(rescale=posterior/value) |>
  ggplot(aes(parameter,rescale,fill=group))+
  geom_boxplot()+
  # geom_text(data=df.n,
  #           mapping=aes(x=threshold,y=value,label=n),
  #           position = position_dodge(width=0.75),
...
threshold_post |> 
  ...
  ggplot(aes(parameter,inflex,fill=group))+
  geom_boxplot()+
  # geom_text(data=df.n,
  #           mapping=aes(x=threshold,y=value,label=n),
  #           position = position_dodge(width=0.75),
