...
# correction of data
df.select[df.select$sp.ind=="Bepe","LT50"] <- -23
```
...
### Quality of soil depth data
```{r SoilDepth}
rast.depth=rast("data/STU_EU_Layers/STU_EU_DEPTH_ROOTS.rst")
names(rast.depth)="depth"
rast.depth=classify(rast.depth, cbind(0, NA))
as.data.frame(rast.depth,xy=TRUE) %>% 
  mutate(depth=as.factor(depth)) %>% 
  ggplot(aes(x=x,y=y,fill=depth))+
  geom_tile()+
  theme_bw() +
  theme(axis.title=element_blank())+
  labs(fill="Soil depth (cm)")+
  scale_fill_brewer(palette="RdYlBu")+
  coord_equal()
```

### Comparison of old method (horizon weighted with width) and new method (SUREAU)
```{r psimincomparedweight, fig.cap="Map of minimum soil potential using soil horizons weighting with horizons width or SUREAU"}
tar_load(psi_ESDACs_100)
tar_load(psihormonth_100)
psi_ESDACs_100 %>% 
  select(x,y,psi_CB,psi_VG) %>% 
...
as.data.frame(psi_ESDACs_100,xy=TRUE) %>% 
  select(x,y,psi_CB,psi_VG) %>% 
   pivot_longer(cols=colnames(.)[c(-1,-2)]) %>% 
      filter(value>-10000000) %>% 
      ggplot(aes(-value/1000,color=name))+
        geom_density()+
        theme_bw() +
        theme(axis.title=element_blank(),
              legend.key.size = unit(0.5,"cm"),
              legend.position="bottom")+
        labs(color="Potentiel (MPa)")+
        #scale_color_brewer(palette="RdYlBu")+
        scale_x_log10()+
        geom_vline(xintercept = c(0.1,1,100))

psihormonth_100=rast(psihormonth_100[,c("x","y","psi")],crs="epsg:4326")
psi_ESDACs_100=project(rast(psi_ESDACs_100[,c("x","y","psi_VG")],crs="epsg:4326"),psihormonth_100,method="near")
as.data.frame(c(psihormonth_100,psi_ESDACs_100),xy=TRUE) %>% 
  mutate(across(.cols=matches(c("psi")),
                    ~ cut(.,
                          breaks=c(-Inf, -50000,-25000, -10000, -5000,-3000,-1000,-500,-300,-200,-100,Inf), 
                          labels=c("<-50MPa","-50<Psi<-25MPa","-25<Psi<-10MPa","-10<Psi<-5MPa",
...
### Comparison of the use of depth threshold/real depth, monthly/daily timestep
...
```{r HSM,fig.cap="Safety margins for 5 species, computed with the 3 different methods"}
#,fig.width=7
tar_load(europe)
...
```{r CompareFrostIndex,fig.cap="Comparison of the 3 frost index",fig.width=15}
...
names(rast.min)="min"
rast.quant=crop(min(rast("data/CHELSA/CHELSA_EUR11_tasmin_month_quant05_19802005.nc"),na.rm = FALSE),europe,mask=TRUE)
...
```{r SafetyMarginsLT50winter,fig.cap="Safety margins of the 5 selected species, computed for the 3 frost index and LT50 of winter"} 
#,fig.width=12
...
```{r SafetyMarginsFrostQuant,fig.cap="Safety margins of the 5 selected species, computed for the 2 frost index and LT50 of spring"}
#,fig.width=7
  ...
  tryCatch(
      {
       spdistrib=read_sf(dsn=file.path("data",
        ...
        print(
          frost.fdg.gather %>% 
            mutate(fdg=-5+30*(5+df.select$LT50[i])/(2*fdg)) %>% # LT50 in spring
            pivot_longer(cols=c("quant","min")) %>% 
            mutate(fsm=cut(value-fdg,
                           breaks=c(-Inf, -15,-10, -5,0,5,10,15,Inf), #,20,25,30,35
                           labels=c("<-15", "-15<FSM<-10","-10/-5","-5/0",
                                    "0/5","5/10","10/15",">15")))%>% #,"20/25","25/30","30/35",">35"
            ggplot() +
            geom_tile(aes(x=x,y=y,fill=fsm))+
            geom_sf(data=spdistrib,
                    fill=alpha("grey",0.4),
...
### LT50 spring for mean fdg per species, and frost index at each location
  ...
  tryCatch(
      {
        ...
        ## Build graphs
        print(
          as.data.frame(rast.temp,xy=TRUE) %>% 
          mutate(fsm=cut(tmin-df.fdg.sp[df.fdg.sp$species.binomial==df.select$species.binomial[i],"LT50.sp.spring"][[1]],
                             breaks=c(-Inf, -15,-10, -5,0,5,10,15,20,25,30,35,Inf),
                             labels=c("<-15", "-15<FSM<-10","-10/-5","-5/0",
                                      "0/5","5/10","10/15","15/20","20/25",
                                      "25/30","30/35",">35"))) %>% 
