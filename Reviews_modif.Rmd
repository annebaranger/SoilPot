...
df.loc.new<-as.data.frame(spTransform(df.loc, CRS("+proj=lcc +lat_0=50 +lon_0=8 +lat_1=50 +lat_2=50 +x_0=0 +y_0=0 +R=6371229 +units=m +no_defs"))) 
...
df.loc.cerra<-cbind(df.loc,
              terra::extract(rast(cfile$file.h2)[[year(time)==year]],
                             data.frame(x=df.loc$x,y=df.loc$y)) [-1])
colnames(df.loc.cerra)[!grepl("^x|^y_cat$|^y",colnames(df.loc.cerra))]=as.character(time[year(time)==year])
df.loc.cerra |> 
